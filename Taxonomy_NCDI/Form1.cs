...
namespace Taxonomy_NCDI
{
    public partial class Form1 : Form
    {
        ...
        {
            string namesFile =  FileString.OpenAs("Select the NCBI taxonomy names.dmp file", "*.dmp|*.dmp");// "J:\\ncbiTaxonomyData\\names.dmp";
        ...
        private void btnCombine_Click(object sender, EventArgs e)
        {
           ...
           cM.ShowDialog();
            ...
            string countsFile=FileString.OpenAs("Select the name of the counts file", "Tab-delimited text file (*.txt; *.xls)|*.txt;*.xls");
            ...
            try
            {
                fs = new System.IO.StreamReader(countsFile);
                fw = new System.IO.StreamWriter(countsFile.Substring(0, countsFile.LastIndexOf(".")) + "_combined.xls");
                string line = fs.ReadLine();
                fw.WriteLine(line + "\t" + title);
                fw.WriteLine(fs.ReadLine());
                while(fs.Peek() >0)
                {
                    line = fs.ReadLine();
                    string index = line.Substring(0, line.IndexOf("\t"));
                    if (data.ContainsKey(index)==true)
                    { fw.WriteLine(line + "\t" + data[index]); }
                    else
                    { fw.WriteLine(line + "\t" +"No data"); }
