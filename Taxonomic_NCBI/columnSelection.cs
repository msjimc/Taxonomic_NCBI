...
namespace Taxonomic_NCBI
{
    public partial class columnSelection : Form
    {
        ...
        public fieldsToSearch getFieldsToSearch()
        {
            if (firstSet == true)
            {
               fieldsToSearch fts = new fieldsToSearch(first, secondSet, second, chkFirstFromEnd.Checked, firstFirstIndex, firstSecondIndex, chkSecondFromEnd.Checked, secondFirstIndex, secondSecondIndex, itemsCount, chkCombinefields.Checked, chkReverseOrder.Checked, chkIgnoreNones.Checked, chkNCBIClassification.Checked);
                return fts;
            }
            else
            { return null; }
